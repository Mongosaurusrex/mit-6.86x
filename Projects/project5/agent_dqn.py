...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.rand() < epsilon:
        ...
        # Exploit: use model to pick best (action, object)
        with torch.no_grad():
            q_actions, q_objects = model(state_vector)

            # Compute combined Q(s, a, o) = 0.5 * (q_action[a] + q_object[o]) for all combinations
            q_total = (
                q_actions.unsqueeze(1) + q_objects.unsqueeze(0)
            ) * 0.5  # shape: [NUM_ACTIONS, NUM_OBJECTS]

            best_action, best_object = divmod(torch.argmax(q_total).item(), NUM_OBJECTS)

        action_index = best_action
        object_index = best_object
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    with torch.no_grad():
        ...
        target_val = reward if terminal else reward + GAMMA * maxq_next.item()
    target = torch.tensor(target_val, dtype=torch.float32)

    # Get predicted Q-value for current state and chosen (action, object)
