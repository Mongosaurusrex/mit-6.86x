...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    prob = np.log(
        compute_probabilities(X=X, theta=theta, temp_parameter=temp_parameter)
    )
    mask = np.array([[Y[i] == j for i in range(n)] for j in range(k)])
    error = (-1 / n) * np.sum(prob[mask])
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    return (
        ...
        - (
            np.mod(
                get_classification(X=X, theta=theta, temp_parameter=temp_parameter), 3
            )
            == Y
        ).mean()
    )
